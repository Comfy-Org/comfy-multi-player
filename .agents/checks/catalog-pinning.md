...
the catalog/vocabulary by a moving branch instead of a SHA, or bypasses
catalog-corpus-check=verify:corpus, the CI hash-verification gate. Preserve the
one-way-corpus-rule: expected outputs come from the pinned upstream generator
or explicitly documented manual fixtures, never from the applier under test.
Regeneration is a separate manual procedure; CI verifies the retained hashes.
```
...
<!-- claim: catalog-corpus-check=verify:corpus :: .coderabbit.yaml -->
