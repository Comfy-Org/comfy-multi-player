...
day each of those gates landed. `check:coderabbit` asserts that `.coderabbit.yaml`'s
`path_instructions` still match the `<!-- coderabbit-instructions -->` blocks in
`.agents/checks/*.md` that generate them; if it fails, edit the block in the owning
profile and run `npm run gen:coderabbit`, never the YAML directly. The purity gate asserts positively that the declared and resolved production dependency roots are exactly `{yjs}`, and adds a dependency denylist and a bare-Node import probe (issue #22, which raised the missing positive assertion, is closed). `check:imports` covers the same contract one layer down — it cruises the module graph with `.dependency-cruiser.cjs` and asserts the yjs-only and no-Node-builtin boundaries **per source module**, which is what the package-level gate structurally cannot see; it exits `2` (INCONCLUSIVE) rather than green if it analyzed too few modules to mean anything.
