...
The class is **not closed**, and three holes remain, the same three named in `src/applier.ts`'s module JSDoc and in `test/invalid-op-states.test.ts`: values `structuredClone` accepts but Yjs cannot store still mutate-then-throw; a reference cycle is accepted and then makes the document permanently unencodable; and `delete_node` with a non-array `removed_links` deletes the node before it throws, because that precondition is read from the op but evaluated after `mdel` — see #59/#61/#68. (A third, `stampKey`'s post-append `Number(stamp[0])`, was closed as a side effect of hoisting `stampKey` for convergence; it is pinned byte-identical rather than left to drift.)
