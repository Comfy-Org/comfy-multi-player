...
**Enforced by:** `test/convergence.test.ts`, `test/connect-lww.test.ts`, `test/applier.test.ts`, `test/reject-no-mutation.regression.test.ts` (the rejection half: the `assertRejectedWithoutMutation`-driven cases assert `encodeStateAsUpdate` byte identity across a reject AND its retry, since a rejected op never records its `op_id`; a few further cases assert byte identity once without a retry; plus the op-only vectors in BOTH arrival orders on the source and destination axes, where the oracle is the projection or the disposition rather than the bytes — two replicas that applied the same ops in different orders hold different Yjs clocks legitimately), and [`convergence-idempotency`](../.agents/checks/convergence-idempotency.md). Property coverage is tracked in #24.

**Four deliberate exceptions to clause 1, added by schema Amendment A6 (#34).** A `connect` refused by a check that must READ a node — `from_slot`/`to_slot` out of range or not addressing a slot record, an opaque widget destination, or a `grow.inputcount.widget` the catalogue cannot describe — racing that node's deletion resolves differently by arrival order, and under §4 abort-remainder that reaches the projection. Every precondition that depends on the OP ALONE is checked before the delete-wins returns and does NOT carve out. The identical shape recurs in `set_widget` (its `!node` and interior delete-wins returns) and `add_node` (behind `nodes.has(key)`) — §2.5 items 6 and 7. See schema §2.5 items 4-7. This note exists because a convergence carve-out invisible at KA-4 is a carve-out nobody will find; all four are permanent deliberate deviations and are logged in `docs/decisions/EXCEPTIONS.md`.

The class is **not closed**, and four holes remain, the same four named in `src/applier.ts`'s module JSDoc and in `test/invalid-op-states.test.ts`: values `structuredClone` accepts but Yjs cannot store still mutate-then-throw; a reference cycle is accepted and then makes the document permanently unencodable; `delete_node` with a non-array `removed_links` deletes the node before it throws, because that precondition is read from the op but evaluated after `mdel`; and `connect.link_id`/`link_type`, which are copied in with no validation at all, so an `undefined` `link_id` mutates then throws and a `Symbol` one leaves the document unprojectable — see #59/#61/#68. (A further hole, `stampKey`'s post-append `Number(stamp[0])`, was closed as a side effect of hoisting `stampKey` for convergence; it is pinned byte-identical rather than left to drift.)
