...
- **Invalid op field combinations on the wire (#17):** the op types now make twelve invalid states unrepresentable for TypeScript callers, but the runtime validator is unchanged and still ACCEPTS several of them. `test/invalid-op-states.test.ts` group 2 pins the behaviour of each — notably `connect` with both `grow` and a numeric `to_slot`, `set_widget` with `inner_widget` and no `path`, `add_node` whose `node.id` disagrees with `node_id` (the only one that corrupts the projection), and `op_id` accepted whatever its shape despite the uuid4-hex contract that KA-2 makes the final LWW tiebreak. Pinned, not endorsed: closing any of them rejects a currently-accepted payload, so each needs a comfy-cli counterpart and its own ticket.
