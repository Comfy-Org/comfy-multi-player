...
const fixture = JSON.parse(readFileSync(new URL("../fixtures/go-agent-events/contract.json", import.meta.url), "utf8")) as Contract;
...
describe("cloud Go ⇄ cmp TypeScript event schema", () => {
  ...
  it("keeps every Go golden event inside the TypeScript discriminated union", () => {
    const lines = readFileSync(new URL("../fixtures/go-agent-events/golden.jsonl", import.meta.url), "utf8").trim().split("\n");
